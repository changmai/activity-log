...
- **정리 배치 (2026-08-18)**: scripts/tidy.py — 하루 2회(13:00, 23:30) claude CLI로 중복 발화 병합·
  회상 입력 시간 배치·시작/끝 확정. 규칙집(data/tidy-rules.md)에 부합할 때만 자동 적용, 애매하면
  텔레그램 질문 → 답장을 다음 실행 때 규칙으로 학습(질문 점감 구조). 전 변경 감사 로그
  (logs/tidy-audit.jsonl), --rollback RUN_ID로 복원. 진행 중 활동 보호 가드.
